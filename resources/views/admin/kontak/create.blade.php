...
<head>
    ...
    <title>Tambah Pesan Kontak - Admin</title>
                ...
                <div class="card p-4" style="max-width: 700px;">
                    <form action="{{ route('admin.kontak.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="tel" name="telepon" class="form-control @error('telepon') is-invalid @enderror" required>
                            @error('telepon') <span class="invalid-feedback">{{ $message }}</span> @enderror
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Jenis Proyek *</label>
                            <input type="text" name="jenis_proyek" class="form-control @error('jenis_proyek') is-invalid @enderror" required>
                            @error('jenis_proyek') <span class="invalid-feedback">{{ $message }}</span> @enderror
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Pesan *</label>
                            <textarea name="pesan" class="form-control @error('pesan') is-invalid @enderror" rows="6" required></textarea>
                            @error('pesan') <span class="invalid-feedback">{{ $message }}</span> @enderror
                        </div>
                        <div class="d-flex gap-2">
                            <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Simpan</button>
                            <a href="{{ route('admin.kontak.index') }}" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> Kembali</a>
                        </div>
                    </form>
                </div>
            </div>
